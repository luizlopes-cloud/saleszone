...
export async function GET(req: NextRequest) {
  try {
    ...
    const [allDealsRes, paidDealsRes, baserowLeadsRes, mqlDeals, sqlDeals, oppDeals, wonDeals] = await Promise.all([
      ...
      // MQL deals by canal
      fetchAll(admin.from("szs_deals").select("canal, lost_reason").gte("add_time", startDate)),
      // SQL deals by canal
      fetchAll(admin.from("szs_deals").select("canal, lost_reason").gte("qualificacao_date", startDate)),
      // OPP deals by canal
      fetchAll(admin.from("szs_deals").select("canal, lost_reason").gte("reuniao_date", startDate)),
      // WON deals by canal
