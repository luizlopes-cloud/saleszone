-- Update RPC to accept days_back parameter for flexible date filtering
...
AS $$
  ...
  WHERE is_marketing = TRUE
    ...
    AND add_time >= CASE
      WHEN days_back > 0 THEN NOW() - (days_back || ' days')::interval
      ELSE NOW() - (months_back || ' months')::interval
    END
